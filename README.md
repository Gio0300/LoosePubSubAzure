...
The [publisher-subscriber pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/publisher-subscriber) is all about decoupling components. This article focuses on evangelizing the principals and benefits of loosely coupled pub-sub architectures. Our journey will begin by examining a simple example of how the pub-sub pattern functioned in the music industry before digital streaming became the norm. After all, the pub-sub pattern originates from the real-world model of physical media distribution. Then we will translate the real-world example into an abstract sequence diagram. Lastly, we will explore an implementation using [Azure](https://azure.microsoft.com/en-us) resources to demonstrate that the principals can be implemented easily. We are not just talking theory here.
